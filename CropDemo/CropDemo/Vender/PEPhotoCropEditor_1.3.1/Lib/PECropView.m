...
/// freezeSize 为 YES 时，保持图片能填满预设的尺寸。
...
- (void)commonInit
{
    _cropRectView.userInteractionEnabled = NO;
...
- (void)setImageCropRect:(CGRect)imageCropRect
{
    ...
    _zoomScale = _scrollView.zoomScale;
}
...
- (void)setFreezeSize:(BOOL)freezeSize
{
    _freezeSize = freezeSize;
    
    self.cropRectView.freezeSize = freezeSize;
}
...
- (void)setShowsGridMajor:(BOOL)showsGridMajor
{
    ...
    self.cropRectView.showsGridMajor = showsGridMajor;
}
...
{
    if (_freezeSize) {
        if (scrollView.zoomScale < _zoomScale) {
            [scrollView setZoomScale:_zoomScale animated:YES];
        }
    }
}
